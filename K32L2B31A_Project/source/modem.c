...
#define SIZE_BUFFER_COMANDO	100
#define EC25_BYTES_EN_BUFFER_MQTT 100
...
uint32_t 	index_buffer_nuevo_comandio_enviar=0;

void Modem_Rta_Run(void);
int32_t numeroDeBytesDisponiblesEnBufferRx(void);
uint8_t pullByteDesdeBufferCircular(void);

void limpiarBufferComandoTX(){
	for(int i=0;i<SIZE_BUFFER_COMANDO;i++){
		buffer_comando_enviar[i]=0x00;
	}
	index_buffer_nuevo_comandio_enviar=0;
}

void limpiarBufferComandoRX(){
	for(int i=0;i<SIZE_BUFFER_COMANDO;i++){
...
void ec25sensor(float valor_temp, float valor_hum, float lum){
	...
	sprintf(ec25_mensaje_mqtt,"temperatura,%.2f,humedad,%.2f,luminosidad,%.2f\r\n %c",valor_temp,valor_hum, lum, CNTL_Z);
	memcpy(&buffer_comando_enviar[0],ec25_mensaje_mqtt,strlen(ec25_mensaje_mqtt));	//copia mensaje a enviar en buffer TX del EC25
...
//extern float adc_sensor_de_temp;
//extern float adc_sensor_de_pres;
extern float adc_sensor_de_lum;
extern float resultado_temperatura_sth31;
extern float resultado_humedad_sth31;
...
void Modem_Task_Run(void){
	...
	case ST_MOD_PWR_OFF:
		...
		Modem_Rta_Cmd(6,"OK",ST_MOD_PWR_ON,ST_MOD_PWR_OFF); //rx OK
 	break;
	case ST_MOD_APN:
		...
		//Modem_Send_Cmd(buffer_comando_enviar);
		Modem_Send_Cmd("AT+CGDCONT=1,\"IP\",\"web.colombiamovil.com.co\"\r\n"); //tx "AT+CGDCONT=1,"IP",APN_APP
		Modem_Rta_Cmd(TIME_WAIT_RTA_CMD,"OK",ST_MOD_SIM,ST_MOD_APN); 	// rx "OK"
	break;
	case ST_MOD_PWR_ON:
		Modem_Send_Cmd("AT+CFUN=1\r\n");								//tx "AT+CFUN=1"
		Modem_Rta_Cmd(6,"OK",ST_MOD_APN,ST_MOD_PWR_ON); 	//rx "OK"
    break;
 	case ST_MOD_SIM:
	...
	case ST_MOD_OPEN_MQTT:
		Modem_Send_Cmd("AT+QMTOPEN=0,\"20.49.8.242\",1883\r\n"); //tx "AT+QMTOPEN=0,"142.93.88.99",1883"
		Modem_Rta_Cmd(TIME_WAIT_RTA_CMD,"+QMTOPEN: 0,0",ST_MOD_CONN_TOPIC,ST_MOD_OPEN_MQTT);
	break;
	case ST_MOD_CONN_TOPIC:
		//Modem_Send_Cmd("AT+QMTCONN=0,\"modem\"[,\"guest\",\"guest\"]\r\n");	//tx "AT+QMTCONN=0,"TOPICO_APP""
		Modem_Send_Cmd("AT+QMTCONN=0,\"1/sensor\"\r\n");
		Modem_Rta_Cmd(8,"+QMTCONN: 0,0,0",ST_MOD_CONN_PUB,ST_MOD_OPEN_MQTT);
	break;
	case ST_MOD_CONN_PUB:
		Modem_Send_Cmd("AT+QMTPUB=0,0,0,0,\"1/sensor\"\r\n");		//tx "AT+QMTPUB=0,0,0,0,TOPICO_APP"
		Modem_Rta_Cmd(8,">",ST_MOD_PUBLIC_DAT,ST_MOD_OPEN_MQTT);
	break;
	case ST_MOD_PUBLIC_DAT:
			Modem_Send_Cmd(buffer_comando_enviar);
			//printf("temperatura,%.2f,humedad,%.2f,luminosidad,%.2f,\r\n %c",resultado_temperatura_sth31, resultado_humedad_sth31, adc_sensor_de_lum, CNTL_Z);
...
char Modem_Rta_Cmd(uint32_t time2Wait,char *rtaValidaStr,char estadoOk,char estadoErr){
	if(!procesandoComando){
		procesandoComando = 1;
   		limpiarBufferComandoRX();
   		//limpiarBufferComandoTX();
